...
def train(dataset, valid_data, test_data, option):
    ...
    if option.early_stop:
        extend_graph(graph_entity, valid_data, test_data)

    model = TCLMModel(len(dataset.relation2id), option.step, option.length, len(dataset.entity2id),
                  option.tau_1, option.tau_2, option.use_gpu, option.dropout)
    ...
    for e in range(option.max_epoch):
        ...
        for k, batch in enumerate(dataset.batch_iter()):
            ...
            input_x = torch.nn.functional.one_hot(input_x, len(dataset.entity2id)).float().to_sparse()
            if option.use_gpu: input_x = input_x.cuda()
            state = model(input_x, e2triple, triple2e, triple2r, flag, is_training=True)
            ...
            if option.with_constrain:
                con_loss = model.constrain_loss((e + 1) / option.max_epoch, option.lammda)
                print(con_loss)
                loss += torch.sum(con_loss, dim=-1)
        ...
        if option.early_stop:
            mrr, hit_1, hit_3, hit_10 = valid_process(valid_data, dataset, model, e2triple, triple2e,
                                                 triple2r, graph_entity, option)
            if mrr > max_score:
                max_score = mrr
...
if __name__ == '__main__':
    ...
    save_data(option.target_relation, dataset.kg, dataset.entity2id, dataset.relation2id,
              dataset.triple2id, option.exp_dir)
    ...
    train(dataset, valid_data, test_data, option)
    print('Current Time: {}, Cost: {}'.format(time.strftime('%Y-%m-%d %H:%M:%S'), time.time() - start_time))
